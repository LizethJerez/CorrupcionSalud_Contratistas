...
```{r}
variablesAnno <- BaseDatos %>% group_by(adicion, nombre_familia) %>% 
...
indice_IHH_frec_Prd <- BaseDatos %>%
      group_by(adicion, nombre_familia, id_contratista_2) %>%
      summarise(total = n()) %>% 
      group_by(adicion, nombre_familia) %>% 
...
variablesAnno <- merge(x = variablesAnno, y = indice_IHH_frec_Prd, 
                            by.x = c("adicion","nombre_familia"),
                                by.y = c("adicion","nombre_familia"))
...
ggplot(data = variablesAnno, 
       aes(x = log(numero_contratistas), y = log(porcion_procesos_abiertos), color = adicion)) + geom_point()
```
