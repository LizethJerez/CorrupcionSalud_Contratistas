...
write.csv(BaseDatos, "SECOP_I_IPS_Depurada.CSV")
...
for(i in 1:nrow(BaseDatos)){
  ...
  prueba <- BaseDatos[i , "municipio_ejecucion"]
  
  if(str_detect(prueba, ";")){
    BaseDatos[i, "municipio_ejecucion_2"] <-  
       {x <- BaseDatos[i,] %>% select(municipio_ejecucion) %>% str_split(";");
        x[[1]][1]}
  }
...
## nombrar los atributo al final de la limpiezA
